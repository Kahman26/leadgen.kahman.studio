...
# Ниша по умолчанию: туда ложатся объекты, для которых ниша не названа, —
# форма без выбора, импорт без колонки niche. Правила сбора — niche_rules.py.
...
# Коды нужны коду: по ним подключаются правила сбора (niche_rules.py).
SEED = [
...
def find_category(c, niche_id, title):
    ...
    # названия из niche_rules.RULES — по ним и находим новую категорию.
