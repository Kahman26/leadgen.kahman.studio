...
REASONS = {
    ...
    "NO_BOOKING":    ("Нет онлайн-бронирования",     50, 14),
...
def _pitch(code, lead, audit):
    ...
    if code == "NO_BOOKING":
        return (f"На сайте нет онлайн-бронирования — гость должен звонить в рабочее время. "
                f"Половина решает быстрее и бронирует там, где можно оплатить сразу.")
    if code == "NO_HTTPS":
...
def score_lead(lead, audit):
    ...
    if status == "none" or not has_site:
        ...
        if not audit.get("online_booking"):
            problems.append("NO_BOOKING")
            missing.append("Нет онлайн-бронирования на сайте")
